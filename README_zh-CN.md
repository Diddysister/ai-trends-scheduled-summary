...
AI 趋势定时汇总通过 5-Agent 流水线，按每日定时任务（或手动触发）从 Hacker News、Dev.to 及可配置的 Web 源采集 AI 行业资讯，生成结构化的 Markdown 趋势报告，并筛选出当天最适合做成 AI 自媒体内容的 5 个选题。整个流水线通过 SSE 实时推送进度，用户可以看到资讯被逐步采集、筛选、评分、策划选题和撰写的全过程。

- **多源采集** — 从 Hacker News、Dev.to 及可配置 Web 源（通过沙箱浏览器抓取）获取数据
- **5-Agent 流水线** — Curator（筛选）+ Summarizer（摘要）并行执行，随后 Analyst（评分+分类）、TopicPlanner（自媒体选题策划）和 Writer（撰写报告）
...
- **AI 自媒体选题** — TopicPlanner 从当天资讯中筛出最多 5 个高价值选题，并给出选题价值、切入角度、开头钩子、建议形式和目标受众
...
│       ├── _model.ts           # 5-Agent 定义、prompt、流式逻辑
...
5. **TopicPlanner** — 从当天高价值资讯中筛选最多 5 个适合 AI 自媒体表达的选题，输出选题价值、切入角度、开头钩子、建议形式和目标受众。失败时回退到基于 Analyst 分数的代码生成选题。

6. **Writer（逐 token 流式）** — 生成结构化 Markdown 报告，包含固定的 `AI 自媒体选题` 章节，逐 token 流式推送到客户端。实时过滤 `<think>` 标签。连接失败时自动回退到非流式重试。

7. **持久化** — 最终报告保存到 `context.store`（平台 Memory）。不可用时回退到文件系统存储。
...
| `content_topics` | TopicPlanner 输出（当天 AI 自媒体选题） |
